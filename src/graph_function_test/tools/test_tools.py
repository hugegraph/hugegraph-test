...
class Test_tools:
    ...
    def test_tools_get_task(self):
        ...
        :return:
        """
        cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s task-list"
        res = run_shell(cmd)
        stdout, stderr = res.communicate(timeout=120)
        print(' ---> ' + str(stdout) + ' === ' + str(stderr))
        assert res.returncode == 0
        assert str(stdout, 'utf-8').split('\n')[1] == '[%s] Tasks: [' % _cfg.graph_space
    ...
    def test_tools_get_task_limit(self):
        ...
        :return:
        """
        cmd = "./bin/hugegraph --url  %s --space %s --graph %s %s %s task-list --limit 3 "
        res = run_shell(cmd)
        stdout, stderr = res.communicate(timeout=120)
        print(' ---> ' + str(stdout) + ' === ' + str(stderr))
        assert res.returncode == 0
    ...
    def test_tool_get_task_success(self):
        ...
        :return:
        """
        cmd = "./bin/hugegraph --url  %s --space %s --graph %s %s %s task-list  --status success "
        res = run_shell(cmd)
        stdout, stderr = res.communicate(timeout=120)
        print(' ---> ' + str(stdout) + ' === ' + str(stderr))
        assert res.returncode == 0
    ...
    def test_tool_get_mode(self):
        ...
        :return:
        """
        cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s graph-mode-get "
        res = run_shell(cmd)
        stdout, stderr = res.communicate(timeout=120)
        print(' ---> ' + str(stdout) + ' === ' + str(stderr))
        assert res.returncode == 0
        assert str(stdout, 'utf-8').split('\n')[1].startswith('[%s] Graph mode:' % _cfg.graph_space)
    ...
    def test_tool_set_mode_restore(self):
        ...
        :return:
        """
        cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s graph-mode-set -m RESTORING "
        res_restore = run_shell(cmd)
        stdout, stderr = res_restore.communicate(timeout=120)
        print(' ---> ' + str(stdout) + ' === ' + str(stderr))
        assert res_restore.returncode == 0
        assert str(stdout, 'utf-8').split('\n')[1] == "[%s] Set graph '%s' mode to 'RESTORING'" % (
            _cfg.graph_space,
            _cfg.graph_name
        )

        # 清空图模式
        res0 = run_shell("./bin/hugegraph --url %s --space %s --graph %s %s %s graph-mode-set -m NONE ")
    ...
    def test_tool_get_graph(self):
        ...
        :return:
        """
        cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s graph-list "
        res = run_shell(cmd)
        stdout, stderr = res.communicate(timeout=120)
        print(' ---> ' + str(stdout) + ' === ' + str(stderr))
        assert res.returncode == 0
        assert str(stdout, 'utf-8').split('\n')[1] == '[%s] Graphs: [' % _cfg.graph_space
    ...
    def test_tool_clear_graph(self):
        ...
        :return:
        """
        cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s graph-clear"
        res = run_shell(cmd)
        stdout, stderr = res.communicate(timeout=120)
        print(' ---> ' + str(stdout) + ' === ' + str(stderr))
        ...
        assert res.returncode == 0
        assert str(stdout, 'utf-8').split('\n')[1].startswith(
            "[%s] Graph '%s' is cleared" % (_cfg.graph_space, _cfg.graph_name)
        ) and \
               res_v == 0 and \
               res_e == 0

    def test_create_drop_graph(self):
        """
        创建图 ./bin/hugegraph --url http://127.0.0.1:8080 --user admin --password admin --space gs1 graph-create -n hugegraph2 -f ./hugegraph2.properties
        :return:
        """
        # create graph
        cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s graph-create -n testhglxb -f " \
              + current_path + "/../../config/dataset/graph/testhglxb.properties"
        res = run_shell(cmd)
        stdout, stderr = res.communicate(timeout=10)
        print(' ---> ' + str(stdout) + ' === ' + str(stderr))
        assert str(stdout, 'utf-8').split('\n')[1].startswith(
            "[%s] Graph '%s' is created" % (_cfg.graph_space, 'testhglxb')
        )

        # drop graph
        cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s graph-drop "
        res = run_shell(cmd, graph_name='testhglxb')
        stdout, stderr = res.communicate(timeout=10)
        print(' ---> ' + str(stdout) + ' === ' + str(stderr))
        assert str(stdout, 'utf-8').split('\n')[1].startswith(
            "[%s] Graph '%s' is dropped" % (_cfg.graph_space, 'testhglxb')
        )
    ...
    def test_tool_backup_all(self):
        ...
        insert_data()
        # 开始case测试
        cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s backup -t all --directory ./backup" + str(
            int(time.time()))
        res = run_shell(cmd)
        stdout, stderr = res.communicate(timeout=120)
        print(' ---> ' + str(stdout) + ' === ' + str(stderr))
    ...
    def test_tool_backup_vertex(self):
        ...
        insert_data()
        # 开始case测试
        cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s backup " \
    ...
    def test_tool_restore(self):
        ...
        insert_data()
        ### 数据备份
        backup_cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s backup -t all --directory ./" + dir_data
        backup_res = run_shell(backup_cmd)
        backup_res.communicate(timeout=120)
        ### 清空数据
        clear_graph()
        ### 设置图模式
        mode_cmd = "./bin/hugegraph --url  %s --space %s --graph %s %s %s  graph-mode-set -m RESTORING "
        res_mode = run_shell(mode_cmd)
        res_mode.communicate(timeout=120)
        ### 恢复数据
        restore_cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s restore  -t all --directory ./" + dir_data
        ...
        assert res_e == 8
        ### 恢复图模式
        mode_none = "./bin/hugegraph --url  %s --space %s --graph %s %s %s  graph-mode-set -m NONE "
    ...
    def test_tool_execute_gremlin(self):
        ...
        insert_data()
        # 测试 case
        cmd = "./bin/hugegraph --url  %s --space %s --graph %s %s %s  " \
              "gremlin-execute --script 'g.V().count()' "
        res = run_shell(cmd)
        stdout, stderr = res.communicate(timeout=120)
        print(' ---> ' + str(stdout) + ' === ' + str(stderr))
        assert res.returncode == 0 and str(stdout, 'utf-8').split('\n')[2] == '6'
    ...
    def test_tool_execute_gremlin_job(self):
        ...
        insert_data()
        ### 执行gremlin的job任务
        gremlin_cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s " \
        ...
        task_cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s task-get --task-id " + str(task_id)
    ...
    def test_tool_graph_migrate(self):
        ...
        cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s migrate " \
              "--target-url %s " \
              "--target-space %s " \
              "--target-graph %s " \
              "%s " \
