...
class TestHubbleSingleShortPath(unittest.TestCase):
    """
    hubble的单源带权最短路径算法API
    """
    ...
    def test_singleShortPath(self):
        ...
        code, res = Traverser.SingleShortPath(body=body, graph_id=graph_id)
        print(code, res)
        self.assertEqual(code, 200, "响应状态码不正确")
        self.assertEqual(res['status'], 200, "单源带权最短路径算法返回状态码不正确")
