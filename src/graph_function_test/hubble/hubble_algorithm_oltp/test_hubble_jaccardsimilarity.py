...
class TestHubbleJaccardSimilarity(unittest.TestCase):
    """
    hubble的jaccard相似度算法API
    """
    ...
    def test_jaccardSimilarity(self):
        """
        Jaccard相似度
        """
        graph_id = ID.get_graph_id()
        ...
        code, res = Traverser.JaccardSimilarity(body=body, graph_id=graph_id)
        print(code, res)
        self.assertEqual(code, 200, "响应状态码不正确")
        self.assertEqual(res['status'], 200, "Jaccard相似度算法返回状态码不正确")
