...
class TestHubbleCustomizedPaths(unittest.TestCase):
    """
    hubble的自定义路径算法API
    """
    ...
    def test_customizedPaths(self):
        ...
        code, res = Traverser.CustomizedPaths(body=body, graph_id=graph_id)
        print(code, res)
        self.assertEqual(code, 200, "响应状态码不正确")
        self.assertEqual(res['status'], 200, "自定义路径算法返回状态码不正确")
