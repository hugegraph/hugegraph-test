...
class TestHubbleFusiformSimilarity(unittest.TestCase):
    """
    hubble的模型相似度算法API
    """
    ...
    def test_fusiformSimilarity(self):
        """
        模型相似度算法
        """
        graph_id = ID.get_graph_id()
        ...
        code, res = Traverser.FusiformSimilarity(body=body, graph_id=graph_id)
        print(code, res)
        self.assertEqual(code, 200, "响应状态码不正确")
        self.assertEqual(res['status'], 200, "模型相似度算法算法返回状态码不正确")
