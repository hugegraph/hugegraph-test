...
class TestHubbleWeightedShortPath(unittest.TestCase):
    """
    hubble的带权最短路径算法API
    """
    ...
    def test_weightedShortPath(self):
        """
        带权最短路径
        """
        graph_id = ID.get_graph_id()
        ...
        code, res = Traverser.WeightedShortPath(body=body, graph_id=graph_id)
        print(code, res)
        self.assertEqual(code, 200, "响应状态码不正确")
        self.assertEqual(res['status'], 200, "带权最短路径算法返回状态码不正确")
