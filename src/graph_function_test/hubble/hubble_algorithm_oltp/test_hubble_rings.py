...
class TestHubbleRings(unittest.TestCase):
    """
    hubble的环路检测算法API
    """
    ...
    def test_rings(self):
        ...
        code, res = Traverser.Rings(body=body, graph_id=graph_id)
        print(code, res)
        self.assertEqual(code, 200, "响应状态码不正确")
        self.assertEqual(res['status'], 200, "环路检测算法返回状态码不正确")
    ...
    def test_rings_notExistId(self):
        """
        环路检测
        """
        graph_id = ID.get_graph_id()
