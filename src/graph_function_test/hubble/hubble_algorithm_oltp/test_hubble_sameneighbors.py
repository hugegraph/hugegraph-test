...
class TestHubbleSameNeighbors(unittest.TestCase):
    """
    hubble的共同邻居算法API
    """
    ...
    def test_sameNeighbors(self):
        ...
        code, res = Traverser.SameNeighbors(body=body, graph_id=graph_id)
        print(code, res)
        self.assertEqual(code, 200, "响应状态码不正确")
        self.assertEqual(res['status'], 200, "共同邻居算法返回状态码不正确")
