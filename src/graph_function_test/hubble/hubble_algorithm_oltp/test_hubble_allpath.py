...
class TestHubbleAllPath(unittest.TestCase):
    """
    hubble的所有路径算法API
    """
    ...
    def test_normal(self):
        ...
        code, res = Traverser.AllPath(body=body, graph_id=graph_id)
        print(code, res)
        self.assertEqual(code, 200, "响应状态码不正确")
        self.assertEqual(res['status'], 200, "所有路径算法返回状态码不正确")
    ...
    def test_sourceID_not_exit(self):
        """
        所有路径
        """
        graph_id = ID.get_graph_id()
        ...
        print(code, res)
        self.assertEqual(code, 200, "响应状态码不正确")
        self.assertEqual(res['status'], 400, "所有路径算法返回状态码不正确")
        # self.assertEqual(res['data']['records'][0]['task_name'], body["content"], "非gremlin异步任务或者异步任务内容有误")
        # self.assertEqual(res['data']['records'][0]['task_type'], "gremlin", "非gremlin异步任务或者异步任务执行失败")
