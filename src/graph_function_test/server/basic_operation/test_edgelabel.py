...
def test_create_edgeLabel_text_link_text():
    ...
    if init_graph():
        code, res = Schema().create_edgeLabel(body=body, auth=auth)
        assert code == 201
...
def test_append_edgeLabel_proeprty_userdata():
    ...
    code, res = Schema().update_edgeLabel(
...
def test_get_edgeLabel_by_name():
    ...
    # eliminate property && userdata
    code, res = Schema().get_edgeLabel_by_name(property_name="link", auth=auth)
    print(code, res)
    assert 200
    assert res['name'] == 'link'
...
def test_delete_edgeLabel_by_name():
    ...
    # eliminate property && userdata
    code, res = Schema().delete_edgeLabel(name="link", auth=auth)
    print(code, res)
    assert 202
