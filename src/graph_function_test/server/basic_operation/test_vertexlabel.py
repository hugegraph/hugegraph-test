...
def init_graph():
    ...
    if _cfg.server_backend == 'cassandra':
        ...
        Graph().put_clear_graphs(json={"action": "clear", "clear_schema": True})
...
def test_create_vertexlabel_default_id():
    ...
    if init_graph():
        code, res = Schema().create_vertexLabel(body=body, auth=auth)
        assert code == 201
...
def test_update_vertexlabel_append_properties_userdata():
    ...
    if init_graph():
        code, res = Schema().update_vertexLabel(
            property_name=property_name,
            param_json={"action": "append"},
            body=body,
            auth=auth
        )
        print(code, res)
...
def test_get_vertexlabel_by_name():
    ...
    code, res = Schema().get_vertexLabel_by_name(name=property_name, auth=auth)
    print(code, res)
    assert code == 200
...
def test_delete_vertexlabel_by_name():
    ...
    code, res = Schema().delete_vertexLabel(name=property_name, auth=auth)
    print(code, res)
    assert code == 202
