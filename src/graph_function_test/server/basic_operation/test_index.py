...
def test_create_range_vertex_indexlabel():
    ...
    code, res = Schema().create_index(body=body, auth=auth)
    print(code, res)
    assert code == 202
    task_id = res['task_id']
...
def test_create_range_vertex_indexlabel_nullableKeys():
    """
    int类型为空的属性 & range索引 & vertexlabel
    """
    init_graph()
    query = "graph.schema().propertyKey('name').asText().ifNotExist().create();" \
            "graph.schema().propertyKey('age').asInt().ifNotExist().create();" \
            "graph.schema().propertyKey('date').asDate().ifNotExist().create();" \
            "graph.schema().vertexLabel('person').properties('name', 'age')" \
            ".primaryKeys('name').nullableKeys('age').ifNotExist().create();" \
            "graph.schema().edgeLabel('knows').sourceLabel('person').targetLabel('person')" \
            ".properties('date').ifNotExist().create()"
    code, res = Gremlin().gremlin_post(query, auth=auth)
    assert code == 200
...
def test_create_range_edge_indexlabel_nullableKeys():
    """
    date类型为空的属性 & range索引 & edgelabel
    """
    init_graph()
    query = "graph.schema().propertyKey('name').asText().ifNotExist().create();" \
            "graph.schema().propertyKey('age').asInt().ifNotExist().create();" \
            ...
            ".primaryKeys('name').ifNotExist().create();" \
            "graph.schema().edgeLabel('knows').sourceLabel('person').targetLabel('person')" \
            ".properties('date').nullableKeys('date').ifNotExist().create()"
    code, res = Gremlin().gremlin_post(query, auth=auth)
    assert code == 200
...
def test_create_secondary_vertex_indexlabel_nullableKeys():
    """
    int类型为空的属性 & secondary索引 & vertexlabel
    """
    init_graph()
    query = "graph.schema().propertyKey('name').asText().ifNotExist().create();" \
            "graph.schema().propertyKey('age').asInt().ifNotExist().create();" \
...
def test_create_secondary_edge_indexlabel():
    """
    date类型的属性 & secondary索引 & edgelabel
    """
    init_graph()
    query = "graph.schema().propertyKey('name').asText().ifNotExist().create();" \
            "graph.schema().propertyKey('age').asInt().ifNotExist().create();" \
...
def test_create_secondary_edge_indexlabel_nullableKeys():
    """
    date类型为空的属性 & secondary索引 & edgelabel
    """
    init_graph()
    query = "graph.schema().propertyKey('name').asText().ifNotExist().create();" \
            "graph.schema().propertyKey('age').asInt().ifNotExist().create();" \
...
def test_create_search_vertex_indexlabel_int_error_nullableKeys():
    """
    int类型为空的的属性 & search索引 & vertexlabel
    """
    init_graph()
    query = "graph.schema().propertyKey('name').asText().ifNotExist().create();" \
            "graph.schema().propertyKey('age').asInt().ifNotExist().create();" \
...
def test_create_search_vertex_indexlabel_text():
    """
    string类型的属性 & search索引 & vertexlabel
    """
    init_graph()
    query = "graph.schema().propertyKey('name').asText().ifNotExist().create();" \
            "graph.schema().propertyKey('age').asInt().ifNotExist().create();" \
...
def test_create_search_vertex_indexlabel_text_nullableKeys():
    """
    string类型为空的属性 & search索引 & vertexlabel
    """
    init_graph()
    query = "graph.schema().propertyKey('name').asText().ifNotExist().create();" \
            "graph.schema().propertyKey('age').asInt().ifNotExist().create();" \
            ...
            "graph.schema().propertyKey('date').asDate().ifNotExist().create();" \
            "graph.schema().vertexLabel('person').properties('name', 'age', 'address')" \
            ".primaryKeys('name').nullableKeys('address').ifNotExist().create();" \
            "graph.schema().edgeLabel('knows').sourceLabel('person').targetLabel('person')" \
            ".properties('date', 'address').ifNotExist().create()"
    code, res = Gremlin().gremlin_post(query, auth=auth)
    assert code == 200
...
def test_create_search_edge_indexlabel_text():
    """
    string类型的属性 & search索引 & edgelabel
    """
    init_graph()
    query = "graph.schema().propertyKey('name').asText().ifNotExist().create();" \
            "graph.schema().propertyKey('age').asInt().ifNotExist().create();" \
...
def test_create_search_edge_indexlabel_text_nullableKeys():
    """
    string类型为空的属性 & search索引 & edgelabel
    """
    init_graph()
    query = "graph.schema().propertyKey('name').asText().ifNotExist().create();" \
            "graph.schema().propertyKey('age').asInt().ifNotExist().create();" \
            ...
            ".primaryKeys('name').ifNotExist().create();" \
            "graph.schema().edgeLabel('knows').sourceLabel('person').targetLabel('person')" \
            ".properties('date', 'address').nullableKeys('address').ifNotExist().create()"
    code, res = Gremlin().gremlin_post(query, auth=auth)
    assert code == 200
...
def test_create_shard_vertex_indexlabel_nullableKeys():
    """
    int类型的属性 & shard索引 & vertexlabel
    """
    init_graph()
...
def test_create_shard_edge_indexlabel_nullableKeys():
    """
    date类型的属性 & shard索引 & edgelabel
    """
    init_graph()
...
def test_create_unique_vertex_indexlabel_nullableKeys():
    """
    int类型的属性 & unique索引 & vertexlabel
    """
    init_graph()
...
def test_create_unique_edge_indexlabel_nullableKeys():
    """
    date类型的属性 & unique索引 & edgelabel
    """
    init_graph()
...
def test_get_indexlabel_by_name():
    ...
    code, res = Schema().get_index_by_name(name=index_name, auth=auth)
    print(code, res)
    assert code == 200
...
def test_delete_indexlabel():
    ...
    index_name = 'rangeByAgeByVertex'
    code, res = Schema().delete_index(name=index_name, auth=auth)
    print(code, res)
    assert code == 202
    task_id = res['task_id']
    result = get_task_res(task_id, 120, auth=auth)
