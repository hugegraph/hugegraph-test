...
def test_create_vertex_batch():
    ...
    code, res = Vertex().create_batch_vertex(body, auth=auth)
    print(code, res)
    ### 断言
    assert code == 201
...
def test_update_vertex_single():
    ...
    code, res = Vertex().update_vertex_property(v_id, action, body, auth=auth)
    print(code, res)
    assert code == 200
    assert res['properties']['age'] == 39


@pytest.mark.skip(reason='使用smaller、bigger策略进行属性值更新，有bug')
def test_update_vertex_batch():
    ...
    code, res = Vertex().update_vertex_batch_property(body, auth=auth)
    print(code, res)
...
def test_get_vertex_by_id():
    ...
    v_id = '"1:marko"'
    code, res = Vertex().get_vertex_by_id(v_id, auth=auth)
    print(code, res)
    assert code == 200
    assert res['properties'] == {'name': 'marko', 'age': 29, 'date': '2021-02-07 00:00:00.000'}


def test_get_vertex_by_id_01():
    """
    通过 id 删除 vertex
    """
    init_graph()
    ...
    v_id = '"1:marko"'
    code, res = Vertex().delete_vertex(v_id, auth=auth)
    print(code, res)
    assert code == 204
