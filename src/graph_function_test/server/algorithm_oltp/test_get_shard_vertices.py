...
class TestShardVertex(unittest.TestCase):
    ...
    def test_reqiured_params(self):
        ...
        :return:
        """
        code, res = Traverser().get_vertex_shard(param_json={"split_size": 67108864}, auth=auth)
        print(code, res)
        self.assertEqual(code, 200)

        json = {'start': res['shards'][0]['start'], 'end': res['shards'][0]['end']}
        code, res = Traverser().get_shard_vertex(param_json=json, auth=auth)
        print(code, res)
        self.assertEqual(code, 200)
