    ...
    with open(file_name, 'r') as f:
        for line in f:
            ...
            if line.startswith('graph'):
                code, res = Gremlin().gremlin_post(each_line, auth=auth)
                ...
                if n == 200:
                    ...
                    n += 1
                    gremlin_str += each_line
        code, res = Gremlin().gremlin_post(gremlin_str, auth=auth)
        print('++++++++++++++++++++++++++++')
        print(code, res)
...
class InsertData:
    ...
    def __init__(self, part_cmd=None, gremlin=None, schema=None, struct=None, dir=None):
        ...
        if self.space:
            self.space_cmd = " --graphspace %s " % self.space
        else:
            self.space_cmd = ""

        if self.is_auth:
            key = list(self.auth.keys())[0]
            self.auth_cmd = " --username %s  --password %s " % (key, self.auth[key])
        else:
            self.auth_cmd = ""
