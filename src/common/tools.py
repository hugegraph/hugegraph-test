...
def run_shell(cmd, graph_name=None, graph_host=None, graph_port=None, graph_space=None):
    """
    执行脚本
    :return:
    """
    ...
    auth_cmd = ''
    if _cfg.is_auth:
        key = list(_cfg.admin_password.keys())[0]
        auth_cmd = ' --user %s --password %s ' % (key, _cfg.admin_password[key])
    ...
    if "migrate" in cmd:
        ...
        # print(cmd)
        run_cmd = cmd % (url, graph_space, graph_name, protocol_cmd, auth_cmd)
...
def clear_graph(graph_name=None, graph_host=None, graph_port=None):
    ...
    res = run_shell(
        "./bin/hugegraph --url %s --space %s --graph %s %s %s graph-clear",
...
def target_insert_data():
    ...
    run_cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s gremlin-execute " \
              ...
              % (url, _cfg.tools_target_space, _cfg.tools_target_graph, target_protocol_cmd, target_auth_cmd)
    print(run_cmd)
    res = subprocess.Popen('cd %s && %s' % (tools_path, run_cmd),
                           shell=True,
                           stdout=subprocess.PIPE,
...
def target_clear_graph():
    ...
    target_auth_cmd = ''
    if _cfg.tools_is_auth:
        target_auth_cmd = ' --user admin --password %s ' % _cfg.tools_target_auth['admin']

    url = target_protocol + '://' + _cfg.tools_target_host + ':' + str(_cfg.tools_target_port)

    run_cmd = "./bin/hugegraph --url %s --space %s --graph %s %s %s graph-clear " \
              % (url, _cfg.tools_target_space, _cfg.tools_target_graph, target_protocol_cmd, target_auth_cmd)
    print('run_cmd: ' + run_cmd)
    res = subprocess.Popen('cd %s && %s' % (tools_path, run_cmd),
                           shell=True,
